from math import cos, fabs, sin, radians
g_l = 1.62
M = 2150
v_p = 3660
a_max = 29.43
...
def is_right_vielosity(v_y, v_x):
    if v_x == 0:
        ...
        print(budogba)
        return (- v_y / v_x - pow(pow(v_y / v_x, 2) + 2 * g_l * fabs(H_0 - 50) / pow(v_x, 2), 0.5)) / (-1 * g_l / pow(v_x, 2)) + L_0 <= r
...
while H_0 > H_1 or v_y0 > 0:
    if L_0 < L_min - 10:
        ...
        m -= delta_m * time
        v_x, v_y, L, H = autopilot(alpha, delta_m, time)
        L_min = r + 5 - pow(v_x0, 2) / (a * pow(2, 0.5))
    elif v_x0 > 0:
        alpha = -45
        delta_m = (M / v_p) * (a_max + g_l)
        m -= delta_m * time
        ...
        m -= delta_m * time
        v_x, v_y, L, H = autopilot(alpha, delta_m, time)
    print(f"Скорость по x: {v_x0}, скорость по y: {v_y0}, координата x: {H_0}, координата y: {L_0}, угол: {alpha}, расход топлива: {delta_m}, время: {time}.")
    v_x0, v_y0, L_0, H_0 = v_x, v_y, L, H
